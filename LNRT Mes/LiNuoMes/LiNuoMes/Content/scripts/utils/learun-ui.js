...
tabiframeId = function () {
    var iframeId = top.$(".body").attr("id");
    return iframeId;
}
$.fn.ComboBox = function (options) {
    //options参数：description,height,width,allowSearch,url,param,data,backa01
    var $select = $(this);
    if (!$select.attr('id')) {
        return false;
    }
...
$.fn.panginationEx = function (options) {
    ...
    options.remote = {
        ...
        totalName: 'records'              //指定返回数据的总数据量的字段名jsonca2 
    }
    $pager.page(options);
}
...
$.currentIframe = function () {
    console.log(top.frames[tabiframeId()]);
    //(top.frames[tabiframeId()].contentWindow);
