        ...
        public static string GetSecondMaintenceInfo()
        {
            ...
            using (var conn = new SqlConnection(ConfigurationManager.ConnectionStrings["ELCO_ConnectionString"].ToString()))
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    {
                        string str3 = "select PmPlanCode,PmPlanName from Equ_PmPlanList where PmLevel='二级保养' and CONVERT(varchar(100), GETDATE(), 23) between  DATEADD(DAY,-PmPreAlarmDates, PmFirstDate) and  DATEADD(DAY,PmPreAlarmDates,PmFirstDate) and PmPlanCode not in (select PmPlanCode from Equ_PmRecordList where PmLevel='二级保养')";
                        cmd.CommandType = CommandType.Text;
