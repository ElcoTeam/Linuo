...
{
    public partial class MaterialPullforline : System.Web.UI.Page
    {
        ...
        public static string GetProduceInfo()
        {
            ...
            using (var conn = new SqlConnection(ConfigurationManager.ConnectionStrings["ELCO_ConnectionString"].ToString()))
            {
                ...
                conn.Open();
                cmd.Connection = conn;
                string str1 = "select * from AIO_Materialpullforline";
        ...
        public static string DeletePullInfo(string no)
        {
            using (var conn = new SqlConnection(ConfigurationManager.ConnectionStrings["ELCO_ConnectionString"].ToString()))
            {
                ...
                try
                {
                    ...
                    cmd.Connection = conn;
                    string str1 = "delete  from AIO_Materialpullforline where No='" + no.ToString().Trim() + "'";
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = str1;
                    cmd.ExecuteNonQuery();
                    transaction.Commit();
        ...
        public static string ConfirmPullInfo(string no)
        {
            using (var conn = new SqlConnection(ConfigurationManager.ConnectionStrings["ELCO_ConnectionString"].ToString()))
            {
                ...
                try
                {
                    ...
                    cmd.Transaction = transaction;
                    cmd.Connection = conn;
                    string str1 = "update  from AIO_Materialpullforline set PullState='1' where No='" + no.ToString().Trim() + "'";
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = str1;
                    cmd.ExecuteNonQuery();
                    transaction.Commit();
