...
{
    /// <summary>
    /// GetMaterialpullforwarehouse 的摘要说明
    /// </summary>
    public class GetMaterialpullforwarehouse : IHttpHandler
    {
        ...
        public string GetDataJson()
        {
            ...
            if (dt != null)
            {
                ...
                for (int j = index; j < pageSize + index && j < totalRecord; j++)
                {
                    ...
                    //strJson += "\"" + dt.Rows[j]["EmpNo"].ToString() != "" ? dt.Rows[j]["EmpNo"].ToString() : dt.Rows[j]["LoginName"].ToString() + "\",";
        ...
        public DataTable GetUserData(string orderno, string materialCode, string produce)
        {
            ...
            using (var conn = new SqlConnection(ConfigurationManager.ConnectionStrings["ELCO_ConnectionString"].ToString()))
            {
                ...
                conn.Open();
                cmd.Connection = conn;
                str = "select * from AIO_Materialpullforwarehouse where OrderNo like '%" + orderno.Trim() + "%' and MaterialCode like '%" + materialCode.Trim() + "%' and Procedure_Name like '%" + produce.Trim() + "%'";
