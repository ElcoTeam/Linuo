    ...
    $.extend(learun, {
        ...
        excel: {
            init: function () {
                ...
                var moduleObj = top.learun.data.get(["menu", moduleId]);
                if (moduleObj != "" && window.location.href.indexOf(moduleObj.F_UrlAddress) != -1) {
                    //初始化excel导入功能
                    var template = top.learun.data.get(["excelImportTemplate", moduleId]);
                    if (!!template) {
                        if (!!template.entitys) {
                            ...
                            $.each(template.keys, function (i, item) {
                                learun.getDataForm({
                                    ...
                                    success: function (data) {
                                        var btnObj = top.learun.data.get(["authorizeButton", moduleId, function (v) { return v.F_ModuleButtonId == data.templateInfo.F_ModuleBtnId }]);
                                        if (!!btnObj) {
                    ...
                    //初始化excel导出功能
                    var exportTemplate = top.learun.data.get(["excelExportTemplate", moduleId]);
                    if (!!exportTemplate) {
                        console.log(exportTemplate);
                        $.each(exportTemplate, function (i, item) {
                            var btnObj = top.learun.data.get(["authorizeButton", moduleId, function (v) { return v.F_ModuleButtonId == item.F_ModuleBtnId }]);
